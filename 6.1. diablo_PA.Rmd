...
# Create the Y factor from de rownames
...
transcriptomics <- data[, grepl("SOLYC", colnames(data))] # genes are labeled as Solyc...
metabolomics <- data[, grepl("met", colnames(data))] # metabolites are labeled as met...
...
# calculate correlation of physiological and transcriptomics data
print("correlation of physiological and transcriptomics data")
cor(pls1$variates$X, pls1$variates$Y) 

# calculate correlation of physiological and metabolomics data
print("correlation of physiological and metabolomics data")
cor(pls2$variates$X, pls2$variates$Y) 

# calculate correlation of transcriptomics and metabolomics data
...
Amb un disseny establert, es pot generar el model DIABLO inicial. S'utilitzarà un nombre arbitràriament elevat de components (en aquest cas, el màxim que podem utilitzar és ncomp = 3, ja que són les rèpliques de cada grup que tenim).
...
<https://mixomics-users.discourse.group/t/reducing-number-of-features-diablo/1163>
...
network(final.diablo.model, blocks = c(1,2,3),
        color.node = c('darkorchid', 'lightgreen', 'cadetblue2'), cutoff = 1,
        save = 'jpeg', name.save = 'network_PA')
...
my.network = network(final.diablo.model, blocks = c(1,2,3),
        color.node = c('darkorchid', 'lightgreen', 'cadetblue2'), cutoff = 0.95,
        save = 'jpeg', name.save = 'network_PA')
write.graph(my.network$gR, file = "network_PA_0.95.gml", format = "gml")
```
...
```{r}
plotLoadings(final.diablo.model, block = 'transcriptomics', comp = 2, contrib = 'max', method = 'median', title = 'transcriptomics block')
plotLoadings(final.diablo.model, block = 'metabolomics', comp = 2, contrib = 'max', method = 'median', title = 'metabolomics block')
plotLoadings(final.diablo.model, block = 'physiological', comp = 1, contrib = 'max', method = 'median', title = 'physiological block')
...
metabolites_annotation <- read_excel("Summary_metabolites_hormones_2019_luk&not_PA.xlsx",sheet = "annotation")
...
print("Physiological parameters selected by DIABLO in PA")
...
print("Genes selected by DIABLO in PA")
...
print("Metabolites selected by DIABLO in PA")
