...
# Load data
data <- read.csv("/Volumes/ftp/Paula Sole/mixOmics/PA/GOEA/network_PA_0.csv", header=TRUE, sep=",")
...
if (nrow(gse@result) > 0) {
  print(barplot(gse,
                x = "GeneRatio",
                showCategory = min(20, nrow(gse@result))) + 
          ggtitle("Barplot of GO Enrichment"))
  
  # Dotplot
  print(dotplot(gse, showCategory = min(20, nrow(gse@result))) + 
...
pdf(file = "Cnetplot_GOEnrichment_PA_descriptions.pdf")
