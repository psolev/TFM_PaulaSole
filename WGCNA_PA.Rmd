...
#knitr::opts_knit$set(root.dir = '/Volumes/ftp/Paula Sole/processed_data')
knitr::opts_knit$set(root.dir = '~/Desktop/WGCNA/PA')
...
Defineix el directori de treball i amb "stringsAsFactors = FALSE" canviem la opció que per defecte converteix strings (cadenes de caràcters) en factors quan es creen dataframes directament amb data.frame() o com a resultat de l'ús de read.table() per llegir dades tabulars.
...
Data <- read.table("/Volumes/ftp/Paula Sole/processed_data/PA_dataframe_log.txt", header=TRUE, sep="\t", row.names=1)
...
```{r}
# upload trait data
datTraits <- read_excel("/Volumes/ftp/Paula Sole/Summary_metabolites_hormones_2019_luk&not_PA.xlsx", sheet = "hormones")
dim(datTraits)
head(datTraits)
...
-   `minModuleSize` = 100: nombre mínim de gens en un mòdul.
...
Representa el dendrograma del gen amb els colors del mòdul i la importància dels trets a sota.
...
Exportem les xarxes amb més variables per visualitzar-les a cytoscape
...
cyt = exportNetworkToCytoscape(modTOM,
                               edgeFile=paste("PA_CytoEdge",paste(module,collapse="-"),".txt",sep=""), 
                               nodeFile=paste("PA_CytoNode",paste(module,collapse="-"),".txt",sep=""), 
                               weighted = TRUE, 
                               threshold = 0.6,
                               nodeNames=modProbes,
