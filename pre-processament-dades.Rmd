...
En aquest informe, realitzarem el pre-processament les dades de 3 tipus d'anàlisis:
...
L'objetiu d'aquest pre-processament és preparar i classificar les dades per poder-les utilitzar el anàlisis posteriors.
...
Per a realitzar el filtratge de gens amb baixa expressió, utilitzem l'eina bioinformàtica iDEP 2.0, que utilitza el següent fragment de codi:

```         
x <- x[ which( apply( cpm(DGEList(counts = x)),  1, function(y) sum(y>=minCPM)) >=  nLibraries ) , ]
```

Les dades es normalitzen mitjançant la funció cpm a edgeR. Es compta el nombre de mostres per sobre d'un minCPM (en el nostre cas minCPM=0,5) i només es conserven gens amb nivells per sobre de minCPM en almenys nLibraries (en el nostre cas nLibraries=2).

Carreguem l'arxiu resultant de la següent manera:
...
A partir de les llistes de gens diferencialment expressats (tant per teixit com per genotip), creem 2 subsets de les dades ja pre-processades:
...
Estandaritzem els noms de les mostres:
...
Exportem els resultat en arxius de text:
...
Modifiquem els dataframes per adaptar-los a les nostres necessitats:
...
Separem els datasets per Lukullum i notabilis
...
Eliminem les variables no significatives (p-valor \> 0,05)
...
Com que de cada mostra (LUKCT, LUKST, notCT i notST) tenim 4 rèpliques però de transcriptòmica en tenim 3, realitzem una anàlisi de dispersió per seleccionar les 3 rèpliques que s'assemblin més i eliminar les que es desvien més de la mitjana.

Per fer-ho:

1.  Calculem la mitjana de cada mostra.

2.  Calculem la Desviació Absoluta de cada valor respecte a la mitjana (ens donarà una mesura de com de lluny està cada valor de la mitjana)

3.  Ordenem els valors per desviació absoluta de menor a major.

4.  Seleccionem les 3 columnes amb menor desviació.
...
Adaptem el dataframe per poder treballar amb ell de manera adequada:
...
Creem un dataset separat per a cada genotip: un amb les dades de Lukullus i un amb les dades de Notabilis:
...
Creem un dataframe que contingui les dades de PA tant de Lukullus com Notabillis i dels 3 tipus d'anàlisis. Les variables en columnes i les mostres en files. Seguim el mateix procediment per la resta de grups, de manera que obtenim 2 dataframes per teixit (PA i AR) i 2 dataframes per genotip (Lukullus i notabilis).
...
Visualitzem la distribució de les dades entre les diferents mostres mitjançant boxplots:
...
Implementem diferents mètodes i realitzem boxplots per veure com es distribueixen les dades normalitzades.
...
Seguim els mateixos passos de normalització per les dades de AR.
...
Guardem les dades normalitzades:
